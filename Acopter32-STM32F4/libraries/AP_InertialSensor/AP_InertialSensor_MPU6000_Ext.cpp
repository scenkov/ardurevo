...
uint16_t AP_InertialSensor_MPU6000_Ext::_init_sensor( Sample_rate sample_rate )
{
    ...
    hal.scheduler->register_timer_process(AP_HAL_MEMBERPROC(&AP_InertialSensor_MPU6000_Ext::_poll_data));
 ...
 */
void AP_InertialSensor_MPU6000_Ext::_poll_data(void)
{
    if (_data_ready()) {
        if (hal.scheduler->in_timerprocess()) {
...
float AP_InertialSensor_MPU6000_Ext::_temp_to_celsius ( int32_t regval )
{
