...
//const float AP_InertialSensor_MPU6000_Ext::_gyro_scale = (0.0174532 / 32.8);
//#else
const float AP_InertialSensor_MPU6000_Ext::_gyro_scale = (0.0174532 / 16.4);
//#endif
 ...
 */
const uint8_t AP_InertialSensor_MPU6000_Ext::_gyro_data_index[3]  = { 5, 4, 6 };
const uint8_t AP_InertialSensor_MPU6000_Ext::_accel_data_index[3] = { 1, 0, 2 };


#if CONFIG_HAL_BOARD == HAL_BOARD_SMACCM
const int8_t AP_InertialSensor_MPU6000_Ext::_gyro_data_sign[3]  = { 1, -1, 1 };
const int8_t AP_InertialSensor_MPU6000_Ext::_accel_data_sign[3] = { 1, -1, 1 };
...
const uint8_t AP_InertialSensor_MPU6000_Ext::_temp_data_index = 3;

int16_t AP_InertialSensor_MPU6000_Ext::_mpu6000_product_id = AP_PRODUCT_ID_NONE;
AP_HAL::DigitalSource *AP_InertialSensor_MPU6000_Ext::_drdy_pin = NULL;
...
bool AP_InertialSensor_MPU6000_Ext::_dmp_initialised = false;
// high byte of number of elements in fifo buffer
uint8_t AP_InertialSensor_MPU6000_Ext::_fifoCountH;
// low byte of number of elements in fifo buffer
uint8_t AP_InertialSensor_MPU6000_Ext::_fifoCountL;
// holds the 4 quaternions representing attitude taken directly from the DMP
Quaternion AP_InertialSensor_MPU6000_Ext::quaternion;

/* Static SPI device driver */
AP_HAL::SPIDeviceDriver* AP_InertialSensor_MPU6000_Ext::_spi = NULL;
AP_HAL::Semaphore* AP_InertialSensor_MPU6000_Ext::_spi_sem = NULL;
...
AP_InertialSensor_MPU6000_Ext::AP_InertialSensor_MPU6000_Ext() : AP_InertialSensor()
{
    _temp = 0;
    _initialised = false;
...
uint16_t AP_InertialSensor_MPU6000_Ext::_init_sensor( Sample_rate sample_rate )
{
    if (_initialised) return _mpu6000_product_id;
    _initialised = true;

    _spi = hal.spi->device(AP_HAL::SPIDevice_MPU6000_Ext);
 ...
 * status register.
 */
bool AP_InertialSensor_MPU6000_Ext::_data_ready()
{
    if (_drdy_pin) {
    ...
    if (hal.scheduler->in_timerprocess()) {
        ...
        if (got) {
            ...
            hal.scheduler->panic(
                PSTR("PANIC: AP_InertialSensor_MPU6000_Ext::_data_ready failed to "
 ...
 */
void AP_InertialSensor_MPU6000_Ext::_poll_data(uint32_t now)
{
    if (_data_ready()) {
        if (hal.scheduler->in_timerprocess()) {
            ...
            if (got) {
                ...
                hal.scheduler->panic(
                        PSTR("PANIC: AP_InertialSensor_MPU6000_Ext::_poll_data "
 ...
 */
void AP_InertialSensor_MPU6000_Ext::_read_data_from_timerprocess()
{
    ...
    if (!got) { 
        ...
        if (semfail_ctr > 100) {
            hal.scheduler->panic(PSTR("PANIC: failed to take SPI semaphore "
                        "100 times in AP_InertialSensor_MPU6000_Ext::"
                        "_read_data_from_timerprocess"));
        }
        return;
    } else {
...
void AP_InertialSensor_MPU6000_Ext::_read_data_transaction() {
...
uint8_t AP_InertialSensor_MPU6000_Ext::_register_read( uint8_t reg )
{
...
void AP_InertialSensor_MPU6000_Ext::register_write(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
 ...
 */
void AP_InertialSensor_MPU6000_Ext::_set_filter_register(uint8_t filter_hz, uint8_t default_filter)
{
...
bool AP_InertialSensor_MPU6000_Ext::hardware_init(Sample_rate sample_rate)
{
...
float AP_InertialSensor_MPU6000_Ext::_temp_to_celsius ( uint16_t regval )
{
    /* TODO */
    return 20.0;
}
...
float AP_InertialSensor_MPU6000_Ext::get_gyro_drift_rate(void)
{
...
uint16_t AP_InertialSensor_MPU6000_Ext::num_samples_available()
{
    _poll_data(0);
...
void AP_InertialSensor_MPU6000_Ext::_dump_registers(void)
{
...
float AP_InertialSensor_MPU6000_Ext::get_delta_time()
{
//#if CONFIG_HAL_BOARD == HAL_BOARD_VRBRAIN
...
void AP_InertialSensor_MPU6000_Ext::push_gyro_offsets_to_dmp()
{
...
void AP_InertialSensor_MPU6000_Ext::set_dmp_gyro_offsets(int16_t offsetX, int16_t offsetY, int16_t offsetZ)
{
    int16_t aux_int;
...
void AP_InertialSensor_MPU6000_Ext::push_accel_offsets_to_dmp()
{
...
void AP_InertialSensor_MPU6000_Ext::set_dmp_accel_offsets(int16_t offsetX, int16_t offsetY, int16_t offsetZ)
{
    int aux_int;
    uint8_t regs[2];
...
void AP_InertialSensor_MPU6000_Ext::dmp_register_write(uint8_t bank, uint8_t address, uint8_t num_bytes, uint8_t data[])
{
...
void AP_InertialSensor_MPU6000_Ext::dmp_init()
{
...
void AP_InertialSensor_MPU6000_Ext::dmp_reset()
{
...
bool AP_InertialSensor_MPU6000_Ext::FIFO_ready()
{
...
void AP_InertialSensor_MPU6000_Ext::FIFO_reset()
{
    uint8_t temp;
...
void AP_InertialSensor_MPU6000_Ext::FIFO_getPacket()
{
    uint8_t i;
    int16_t q_data[4];
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_gyro_calibration()
{
    uint8_t regs[4];
    regs[0]=0x4C;
    regs[1]=0xCD;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_accel_calibration()
{
    uint8_t regs[6];
    regs[0]=0x00;
    regs[1]=0x00;
...
void AP_InertialSensor_MPU6000_Ext::dmp_apply_endian_accel()
{
    uint8_t regs[4];
    regs[0]=0x00;
    regs[1]=0x00;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_mpu_sensors()
{
    uint8_t regs[6];
    regs[0]=0x0C;
    regs[1]=0xC9;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_bias_from_no_motion()
{
    uint8_t regs[4];
    regs[0]=0x0D;
    regs[1]=0x35;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_bias_none()
{
    uint8_t regs[4];
    regs[0]=0x98;
    regs[1]=0x98;
...
// dmp_set_fifo_interrupt
void AP_InertialSensor_MPU6000_Ext::dmp_set_fifo_interrupt()
{
    uint8_t regs[1];
    regs[0]=0xFE;
...
void AP_InertialSensor_MPU6000_Ext::dmp_send_quaternion()
{
    uint8_t regs[5];
    regs[0]=0xF1;
    regs[1]=0x20;
...
void AP_InertialSensor_MPU6000_Ext::dmp_send_gyro()
{
    uint8_t regs[4];
    regs[0]=0xF1;
    regs[1]=0x28;
...
void AP_InertialSensor_MPU6000_Ext::dmp_send_accel()
{
    uint8_t regs[54];
    regs[0]=0xF1;
    regs[1]=0x28;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_fifo_rate(uint8_t rate)
{
    uint8_t regs[2];
    regs[0]=0x00;
    regs[1]=rate;
...
void AP_InertialSensor_MPU6000_Ext::dmp_set_sensor_fusion_accel_gain(uint8_t gain)
{
    //inv_key_0_96
...
void AP_InertialSensor_MPU6000_Ext::dmp_load_mem()
{
    ...
    for(int i = 0; i < 7; i++) {
        ...
        for(uint8_t j = 0; j < 16; j++) {
            ...
            _spi->transfer(MPUREG_MEM_R_W);
            for(int k = 0; k < 16; k++) {
                uint8_t byteToSend = pgm_read_byte((const prog_char *)&(dmpMem_Ext[i][j][k]));
    ...
    for(uint8_t j = 0; j < 8; j++) {
        ...
        _spi->transfer(MPUREG_MEM_R_W);
        for(int k = 0; k < 16; k++) {
            uint8_t byteToSend = pgm_read_byte((const prog_char *)&(dmpMem_Ext[7][j][k]));
            _spi->transfer((uint8_t) byteToSend);
        }
        _spi->cs_release();
    }
    ...
    _spi->transfer(MPUREG_MEM_R_W);
    for(int k = 0; k < 9; k++) {
        uint8_t byteToSend = pgm_read_byte((const prog_char *)&(dmpMem_Ext[7][8][k]));
        _spi->transfer((uint8_t) byteToSend);
    }
    _spi->cs_release();
}
...
const uint8_t dmpMem_Ext[8][16][16] PROGMEM = {
    {
        {
