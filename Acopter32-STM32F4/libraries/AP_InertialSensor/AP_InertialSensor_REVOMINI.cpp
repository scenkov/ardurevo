 ...
 */
//const float AP_InertialSensor_REVOMINI::_gyro_scale = (0.0174532f / 32.8f);
const float AP_InertialSensor_REVOMINI::_gyro_scale = (0.0174532f / 16.4f);
...
AP_InertialSensor_REVOMINI::AP_InertialSensor_REVOMINI() :
	AP_InertialSensor(),
    _drdy_pin(NULL),
    _initialised(false),
...
uint16_t AP_InertialSensor_REVOMINI::_init_sensor( Sample_rate sample_rate )
{
    ...
    hal.scheduler->register_timer_process(AP_HAL_MEMBERPROC(&AP_InertialSensor_REVOMINI::_poll_data));
...
bool AP_InertialSensor_REVOMINI::wait_for_sample(uint16_t timeout_ms)
{
...
bool AP_InertialSensor_REVOMINI::update( void )
{
 ...
 * status register.
 */
bool AP_InertialSensor_REVOMINI::_data_ready()
{
    if (_drdy_pin) {
 ...
 */
void AP_InertialSensor_REVOMINI::_poll_data(void)
{
    if (hal.scheduler->in_timerprocess()) {
        ...
        if (_spi_sem->take(10)) {
            ...
            hal.scheduler->panic(
                PSTR("PANIC: AP_InertialSensor_REVOMINI::_poll_data "
...
void AP_InertialSensor_REVOMINI::_read_data_transaction() {
...
uint8_t AP_InertialSensor_REVOMINI::_register_read( uint8_t reg )
{
...
void AP_InertialSensor_REVOMINI::_register_write(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
 ...
 */
void AP_InertialSensor_REVOMINI::_set_filter_frequency(uint8_t filter_hz)
{
    if (filter_hz == 0)
...
bool AP_InertialSensor_REVOMINI::_hardware_init(Sample_rate sample_rate)
{
    if (!_spi_sem->take(100)) {
        hal.scheduler->panic(PSTR("MPU6000: Unable to get semaphore"));
    }
...
float AP_InertialSensor_REVOMINI::_temp_to_celsius ( int32_t regval )
{
...
float AP_InertialSensor_REVOMINI::get_gyro_drift_rate(void)
{
...
bool AP_InertialSensor_REVOMINI::_sample_available()
{
...
void AP_InertialSensor_REVOMINI::_dump_registers(void)
{
...
float AP_InertialSensor_REVOMINI::get_delta_time() const
{
