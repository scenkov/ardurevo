 ...
 *  gyro as 32.8 LSB/DPS at scale factor of +/- 1000dps (FS_SEL==2)
 */
...
uint16_t AP_InertialSensor_VRBRAIN::_init_sensor( Sample_rate sample_rate )
{
    ...
    /*onboard MPU*/
    _spi[0] = hal.spi->device(AP_HAL::SPIDevice_MPU6000);
    _spi_sem[0] = _spi[0]->get_semaphore();

    /* external MPU */
    _spi[1] = hal.spi->device(AP_HAL::SPIDevice_MPU6000_Ext);
    _spi_sem[1] = _spi[1]->get_semaphore();

    /* Pin 99 is the data ready pin */
    _drdy_pin[0] = hal.gpio->channel(99); //internal
    _drdy_pin[1] = hal.gpio->channel(91); //external
	...
	bool success = _hardware_init(sample_rate, _spi[i]);
	if(success)
	    {
	    hal.scheduler->delay(5+2);
	    if (!_spi_sem[i]->take(100))
		{
    ...
    hal.scheduler->register_timer_process(AP_HAL_MEMBERPROC(&AP_InertialSensor_VRBRAIN::_poll_data));
 ...
 * status register.
 */
bool AP_InertialSensor_VRBRAIN::_data_ready()
{
    if (_drdy_pin) {
 ...
 */
void AP_InertialSensor_VRBRAIN::_poll_data(void)
{
...
void AP_InertialSensor_VRBRAIN::_read_data_transaction() {
...
uint8_t AP_InertialSensor_VRBRAIN::_register_read( uint8_t reg )
{
...
void AP_InertialSensor_VRBRAIN::_register_write(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
 ...
 */
void AP_InertialSensor_VRBRAIN::_set_filter_register(uint8_t filter_hz, uint8_t default_filter)
{
...
bool AP_InertialSensor_VRBRAIN::_hardware_init(Sample_rate sample_rate)
{
    if (!_spi_sem->take(100)) {
        hal.scheduler->panic(PSTR("MPU6000: Unable to get semaphore"));
    }
...
void AP_InertialSensor_VRBRAIN::_dump_registers(void)
{
