...
void set_throttle_accel_target( int16_t desired_acceleration )
{
    if( g.throttle_accel_enabled ) {
       ...
       Serial.print_P(PSTR("Err: target sent to inactive acc thr controller!\n"));
    }
}
...
get_throttle_land()
{
    ...
    if (current_loc.alt >= LAND_START_ALT) {
        get_throttle_althold(LAND_START_ALT, -abs((int)g.land_speed));
    }else{
        get_throttle_rate_stabilized(-abs((int)g.land_speed));
