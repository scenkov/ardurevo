...
#ifdef AP_BARO_BMP085_DEBUG_ENABLE
#pragma message "*** AP_BARO_BMP085 Debug Enabled ***"
...
AP_Baro_BMP085::AP_Baro_BMP085(HardwareI2C *i2c_d, FastSerial *ser_port)
{
	serPort = ser_port;
	this->_I2Cx = i2c_d;
}
...
bool AP_Baro_BMP085::init( AP_PeriodicProcess * scheduler)
{
	...
	if (_I2Cx->read((uint8_t)BMP085_ADDRESS, (uint8_t)0xAA, (uint8_t)22, (uint8_t *)buff) != 0) {
		healthy = false;
		return false;
	}
...
{
	if (_I2Cx->write((uint8_t)BMP085_ADDRESS, (uint8_t)0xF4, (uint8_t)(0x34+(OVERSAMPLING << 6))) != 0) {
		healthy = false;
	}
}
...
void AP_Baro_BMP085::ReadPress()
{
	...
	if (_I2Cx->read((uint8_t)BMP085_ADDRESS, (uint8_t)0xF6, (uint8_t)3, (uint8_t *)buf) != 0) {
...
{
	if (_I2Cx->write((uint8_t)BMP085_ADDRESS, (uint8_t)0xF4, (uint8_t)0x2E) != 0) {
		healthy = false;
	}
}
...
void AP_Baro_BMP085::ReadTemp()
{
	...
	if (_I2Cx->read((uint8_t)BMP085_ADDRESS, (uint8_t)0xF6, (uint8_t)2, (uint8_t *)buf) != 0) {
        _retry_time = millis() + 1000;
        _I2Cx->setSpeed(false);
		healthy = false;
		return;
