...
#define AP_VAREND	{ AP_PARAM_NONE, "",}
...
public:
    ...
    typedef struct {
        ...
        uint8_t idx : 6; // offset into array types
    } ParamToken;
    ...
    static bool setup(const struct Info *info, uint8_t num_vars, uint16_t eeprom_size,FastSerial *serext);
...
private:
    ...
    static bool check_group_info(const struct GroupInfo *group_info, uint16_t *total_size, uint8_t max_bits, FastSerial *serext);
    static bool duplicate_key(uint8_t vindex, uint8_t key);
    static bool check_var_info(FastSerial *serext);
