...
// #define ENABLE_FASTSERIAL_DEBUG
 ...
 # define serialDebug(fmt, args...) do { if (serPortVar != NULL) { serPortVar->printf("%s:%d: " fmt "\n", __FUNCTION__, __LINE__ , ##args); delay(100); } } while(0)
#else
                                ...
                                FastSerial *serext)
{
    ...
    for (uint8_t i=0;
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            if (ginfo == NULL ||
                !check_group_info(ginfo, total_size, group_shift + _group_level_shift, serext)) {
...
bool AP_Param::check_var_info(FastSerial *serext)
{
    ...
    for (uint8_t i=0; i<_num_vars; i++) {
        ...
        if (type == AP_PARAM_GROUP) {
            ...
            if (group_info == NULL ||
                !check_group_info(group_info, &total_size, 0, serext)) {
            	 serext->printf("Error: check_group_info\n\r");
                return false;
            }
        } else {
    ...
    if (total_size > _eeprom_size) {
        serext->printf("total_size %u exceeds _eeprom_size %u",
...
bool AP_Param::setup(const AP_Param::Info *info, uint8_t num_vars, uint16_t eeprom_size, FastSerial *serext)
{
    struct EEPROM_header hdr;
    ...
    if (!check_var_info(serext)) {
        return false;
    }
    serext->printf("setup %u vars", (unsigned)num_vars);
    ...
    for (uint8_t i=0;
        ...
        if (strcasecmp_P(name, (const prog_char_t *)group_info[i].name) == 0) {
            ...
            *ptype = (enum ap_var_type)type;
            ...
            uint8_t suffix_len = strlen_P((const prog_char_t *)group_info[i].name);
...
void AP_Param::show_all(FastSerial * ser_port)
{
    ParamToken token;
    AP_Param *ap;
    enum ap_var_type type;
