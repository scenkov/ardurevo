...
#LIBMAPLE_PATH := $(shell cd $(SKETCHBOOK)/libraries/AP_HAL_VRBRAIN../libmaple && pwd)
endif
...
HARDWARE_PATH    := $(SRCROOT)/libraries/AP_HAL_VRBRAIN/hardware
HAL_PATH         := $(SRCROOT)/libraries/AP_HAL_VRBRAIN/hardware/hal
STM32_PATH       := $(HARDWARE_PATH)/STM32F4xx_DSP_StdPeriph_Lib_V1.1.0
STM32USB_PATH    := $(HARDWARE_PATH)/STM32_USB_Driver
WIRISH_PATH      := $(SRCROOT)/libraries/AP_HAL_VRBRAIN/wirish
LIBRARIES_PATH   := $(SRCROOT)/libraries
SUPPORT_PATH     := $(SRCROOT)/libraries/AP_HAL_VRBRAIN/support
...
FAMILY := cortex-m4
F_CPU := 168000000L
LINKER := $(SUPPORT_PATH)/flash.ld
HARDWARE_CORE := vrbrain
UPLOADER := dfu-util
USBID := 1EAF:0003
PRODUCT_ID := 0003
...
# Tool options
#
DEFINES         =   -DF_CPU=$(F_CPU) -DMCU_$(MCU) -DBOARD_$(BOARD) -DERROR_LED_PORT=GPIOC -DERROR_LED_PIN=15 -DVECT_TAB_FLASH 
...
ARM_CPUFLAGS        = -mcpu=$(FAMILY) -march=armv7-m -mthumb -DBOARD_vrbrain -DMCU_$(MCU)
$(cpu_flags) -mthumb -static -Wall -Wl,--gc-sections -Wl,--cref -Wl,-Map,$(BUILD_PATH)/$(BOARD).map -T$(LDDIR)/$(LDSCRIPT) -L$(LDDIR)
